# Script de chay Backend trong cua so PowerShell rieng biet
...
# Luu script tam thoi
$backendScriptPath = Join-Path $env:TEMP "start-backend-only.ps1"
...
Write-Host "`nDa mo cua so PowerShell cho Backend:" -ForegroundColor Green
...
Write-Host "`nTip: Dong cua so PowerShell de dung server" -ForegroundColor Gray
